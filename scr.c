...
static void
mvbot(void)					/* move to bottom of screen */
{
...
void
prbotln(char *s)		/* print a line on the bottom of screen */
{
	mvbot();
	if (clrtoeol() == ERR)
...
void
cpscrn(WINDOW *a, WINDOW *b)			/* copy screen a to screen b */
{
...
void
initialize(void)			/* prepare for screen editing */
{
	int	k;
...
void
reset(void)				/* reset the screen to normal mode */
{
  echo();
  nocrmode();
  endwin();
...
void
update(void)					/* refresh the screen */
{
  if (refresh() == ERR)
    {
...
void
placeobj(char c)			/* place a wall, beeper, or space */
{
  if (cy == y && cx == x)
    {
      placed = 0;
...
void
placekarel(int newy, int newx)	/* put karel on screen at new location */
{
...
void
putbeeper(void)					/* put down one beeper */
{
...
void
help(void)					/* print editor help screen */
{
	touchwin(helpscrn);
...
void
savescrn(void)				/* save the screen to a file */
{
	int	k, j;					/* loop indexes	*/
	FILE	*fp;					/* output file	*/
	FILE	*fopen();
...
void
snapshot(void)				/* save screen with the borders */
{
  int	k, j;					/* loop indexes	*/
  FILE	*fp;					/* output file	*/
  FILE	*fopen();
  ...
  if ((fp = fopen("snapshot", "w")) == NULL)
    syserr("can't open file: snapshot", NULL);
  for (k = 0; k < LINES - 2; k++) {
    fputc('|', fp);
    for (j = 1; j < COLS; j++)
      fputc(scrnchr(k, j), fp);
...
int
sideclear(int n)		/* return 1 if side n is clear, 0 otherwise */
{
...
void
movekarel(void)			/* move karel one character forward */
{
  int	newx, newy;
...
void
finish(void)			/* clean up; reset terminal modes, etc. */
{
...
void
pickbeeper(void)		/* pick up one beeper underneath karel */
{
  if (oldch == ' ')
    {
...
void
readscrn(void)				/* read screen in from a file */
{
  int	k, j;				/* loop indexes		*/
  FILE	*fp;				/* screen file		*/
  FILE	*fopen();
...
void
shutoff(char *s)			/* print s on bottom of screen */
{
  char	mesg[BUFSIZ];
...
void
clearline(void)				/* clear bottom line of screen */
{
  mvbot();
  if (clrtoeol() == ERR)
    screrror("clearing bottom line");
...
void
startprog(void)		/* prepare for execution, then call program */
{
...
void
editscr(void)		/* main interactive loop */
{
